...
bool in_vector(int value, const std::vector<int>& vec) {
  ...
  struct Hypers {
    ...
    void UpdateSigma(const Eigen::VectorXd& res) {
      ...
      sigma2 = fmax(1.0 / R::rgamma(shape, 1.0 / rate), 1e-10);
    }
  };
  ...
  double remaining_mass_from_dims(const int* dims,
                                  int d_len,
                                  const double* weights) {
    double rem = 1.0;
    
    for (int i = 0; i < d_len; ++i) {
      rem -= weights[dims[i] - 1];
    }
    
    return fmax(rem, 1e-16);
  }
  
  double log_sum_exp_vec(const std::vector<double>& vals) {
    if (vals.empty()) return R_NegInf;
  ...
  void build_unordered_subset_dp(const int* dims,
                                 int d_len,
                                 const double* weights,
                                 std::vector<double>& logF,
                                 std::vector<double>& sumW) {
    /*
     Model B unordered subset prior.
     ...
     sum_{j in A}
     F(A \ {j}) *
     s_j / (1 - sum_{l in A \ {j}} s_l)
     
     with F(empty) = 1.
     
     logF[mask] stores log F(A_mask).
     sumW[mask] stores sum_{j in A_mask} s_j.
     */
    
    if (d_len < 0) {
      Rf_error("Negative dimension length.");
    }
    
    /*
     Exact DP is O(d * 2^d). This is fine for small/moderate tessellation
     dimension counts, but impossible for very large d.
     */
    if (d_len > 25) {
      Rf_error("Exact unordered Model B prior is too expensive for d > 25.");
    }
    
    int nmask = 1 << d_len;
    
    logF.assign(nmask, R_NegInf);
    sumW.assign(nmask, 0.0);
    
    logF[0] = 0.0;
    
    for (int mask = 1; mask < nmask; ++mask) {
      int least_bit = mask & -mask;
      int prev = mask ^ least_bit;
      int bit_index = 0;
      
      while (((1 << bit_index) & least_bit) == 0) {
        ++bit_index;
      }
      
      sumW[mask] =
        sumW[prev] +
        fmax(weights[dims[bit_index] - 1], 1e-16);
    }
    ...
    for (int mask = 1; mask < nmask; ++mask) {
      ...
      for (int j = 0; j < d_len; ++j) {
        ...
        double s_j = fmax(weights[dims[j] - 1], 1e-16);
        double denom = fmax(1.0 - sumW[prev], 1e-16);
        
        candidates.push_back(logF[prev] + log(s_j) - log(denom));
      }
      
      logF[mask] = log_sum_exp_vec(candidates);
    }
  }
  
  double log_unordered_dimension_prior(const int* dims,
                                       int d_len,
                                       const double* weights) {
    if (d_len == 0) return 0.0;
    
    std::vector<double> logF;
    std::vector<double> sumW;
    
    build_unordered_subset_dp(dims, d_len, weights, logF, sumW);
    
    int full_mask = (1 << d_len) - 1;
    
    return logF[full_mask];
  }
  
  std::vector<int> sample_order_given_unordered_set(const int* dims,
                                                    int d_len,
                                                    const double* weights) {
    /*
     Samples an ordered latent history h conditional on the unordered set D.
     
     P(h | D, s) proportional to
     
     product_t s_{h_t} / (1 - sum_{r<t} s_{h_r})
     
     This is needed for exact geometric augmentation under Model B.
     */
    
    std::vector<int> order;
    
    if (d_len <= 0) return order;
    
    if (d_len > 25) {
      Rf_error("Exact latent-order sampling is too expensive for d > 25.");
    }
    ...
    order.assign(d_len, 0);
    
    int mask = (1 << d_len) - 1;
    
    for (int pos = d_len - 1; pos >= 0; --pos) {
      ...
      for (int j = 0; j < d_len; ++j) {
        ...
        double lp = logF[prev] + log(s_j) - log(denom);
        
        log_probs.push_back(lp);
        candidate_bits.push_back(j);
      }
      
      double norm = log_sum_exp_vec(log_probs);
      double u = unif_rand();
      double cum = 0.0;
      
      int chosen_j = candidate_bits.back();
      
      for (size_t a = 0; a < log_probs.size(); ++a) {
        double pr = exp(log_probs[a] - norm);
        cum += pr;
        
        if (u <= cum) {
          chosen_j = candidate_bits[a];
          break;
        }
      }
      
      order[pos] = dims[chosen_j];
  ...
  double log_varsel_struct_ratio_modelB(const int* old_dim,
                                        int old_d,
                                        const int* new_dim,
                                        int new_d,
                                        const double* weights,
                                        ...
                                        int var_sel_mode) {
    /*
     Model B: unordered active dimension subset.
     
     The prior is:
     p(D | s) = sum over all ordered without-replacement histories.
     
     The storage order of dim/tess is not assigned prior probability.
     */
    ...
    double log_prior_old =
      log_unordered_dimension_prior(old_dim, old_d, weights);
    
    double log_prior_new =
      log_unordered_dimension_prior(new_dim, new_d, weights);
    
    double log_prior_ratio = log_prior_new - log_prior_old;
    
    if (mod == "AD") {
      /*
       AD proposal:
       choose new variable k with s_k / S_notin_old.
       append or insert in storage representation.
       
       This proposal assumes AD appends only, so reverse RD has 1/new_d.
       */
      
      int new_var = new_dim[mod_idx];
      
      double s_new = fmax(weights[new_var - 1], 1e-16);
      double S_notin_old = remaining_mass_from_dims(old_dim, old_d, weights);
      
      double log_q_reverse_over_forward =
        -log(static_cast<double>(new_d))
        + log(S_notin_old)
        - log(s_new);
        
        return log_prior_ratio + log_q_reverse_over_forward;
    }
    
    if (mod == "RD") {
      /*
       RD proposal:
       remove old_dim[mod_idx] uniformly from old_d dimensions.
       
       Reverse AD appends the removed variable, selecting it with
       s_old / S_notin_new.
       */
      
      int old_var = old_dim[mod_idx];
      
      double s_old = fmax(weights[old_var - 1], 1e-16);
      double S_notin_new = remaining_mass_from_dims(new_dim, new_d, weights);
      
      double log_q_reverse_over_forward =
        log(s_old)
        - log(S_notin_new)
        + log(static_cast<double>(old_d));
  ...
  double calc_acceptance_hard(const std::vector<double>& rIjOld,
                              const std::vector<double>& denomOld,
                              const std::vector<double>& rIjNew,
                              const std::vector<double>& denomNew,
                              int old_centres,
                              int new_centres,
                              int d_old,
                              int d_new,
    ...
    double sum_R2_old = 0.0;
    double sum_R2_new = 0.0;
    
    for (size_t i = 0; i < denomOld.size(); ++i) {
      double den = fmax(denomOld[i], 1e-16);
      sum_log_old += log(den);
      sum_R2_old += (rIjOld[i] * rIjOld[i]) / den;
    }
    
    for (size_t i = 0; i < denomNew.size(); ++i) {
      double den = fmax(denomNew[i], 1e-16);
      sum_log_new += log(den);
      sum_R2_new += (rIjNew[i] * rIjNew[i]) / den;
    }
    
    double safe_sigma2 = fmax(sigma2, 1e-10);
    
    double ratio =
      0.5 * (sum_log_old - sum_log_new)
  ...
  double calc_acceptance_soft(const PosteriorStats& StatsOld,
                              const PosteriorStats& StatsNew,
                              int old_c,
                              int new_c,
                              int old_d,
                              int new_d,
    ...
    double ratio =
      ...
      + 0.5 * (StatsOld.log_det_Q - StatsNew.log_det_Q)
  ...
  std::vector<double> sample_mu_hard(int new_centres,
                                     const std::vector<double>& rIjNew,
                                     const std::vector<int>& nIjNew,
                                     double sigma2mu,
                                     double sigma2) {
    ...
    for (int i = 0; i < new_centres; ++i) {
      if (nIjNew[i] == 0) {
        new_mu[i] = norm_rand() * sqrt(sigma2mu);
        continue;
      }
      ...
      new_mu[i] =
        norm_rand() * sqrt(fmax((safe_sigma2 * sigma2mu) / denom, 0.0))
        + ((sigma2mu * rIjNew[i]) / denom);
    }
    
    return new_mu;
  }
  
  double sample_sigma_squared_cpp(int n,
                                  double nu,
                                  double lambda,
                                  const double* p_y,
                                  const double* p_sum) {
    ...
    return fmax(
      1.0 / R::rgamma(
          (nu + n) / 2.0,
          1.0 / ((nu * lambda + sse) / 2.0)
      ),
      1e-10
    );
  }
  
  void calculate_distances_hard(const double* p_data,
                                const double* p_tess,
                                const double* p_tessstar,
                                const double* p_dist,
                                const int* p_old_idx,
                                const int* p_dim,
                                const int* p_dimstar,
    ...
    if (modification == "RC") {
      if (mod_idx > 0) {
        std::memcpy(p_new_dist, p_dist, mod_idx * n_obs * sizeof(double));
      }
      
      if (mod_idx < n_centres_old - 1) {
        std::memcpy(
          p_new_dist + (mod_idx * n_obs),
          p_dist + ((mod_idx + 1) * n_obs),
          ((n_centres_old - 1) - mod_idx) * n_obs * sizeof(double)
        );
      }
      ...
      for (int c = 0; c < n_centres_new; ++c) {
        ...
        for (int i = 0; i < n_obs; ++i) {
          double diff = p_data[i + cov_offset] - tess_val;
          p_new_dist[i + c_offset] =
            p_dist[i + c_offset] + (diff * diff);
        }
      }
      ...
      for (int c = 0; c < n_centres_new; ++c) {
        ...
        for (int i = 0; i < n_obs; ++i) {
          double diff = p_data[i + cov_offset] - tess_val;
          double val = p_dist[i + c_offset] - (diff * diff);
          p_new_dist[i + c_offset] = (val < 0.0) ? 0.0 : val;
        }
      }
      
    } else if (modification == "Swap") {
      int old_cov_offset = (p_dim[mod_idx] - 1) * n_obs;
      int new_cov_offset = (p_dimstar[mod_idx] - 1) * n_obs;
      ...
      for (int c = 0; c < n_centres_new; ++c) {
        ...
        for (int i = 0; i < n_obs; ++i) {
          double o_diff = p_data[i + old_cov_offset] - old_tess_val;
          double n_diff = p_data[i + new_cov_offset] - new_tess_val;
          double val =
            p_dist[i + c_offset] - (o_diff * o_diff) + (n_diff * n_diff);
    ...
    if (modification == "Change") {
      ...
      for (int q = 0; q < n_obs; ++q) {
        if (p_result[q] == 1) {
          current_min_dist[q] = p_new_dist[q];
        }
      }
      ...
      for (int t = 1; t < n_centres_new; ++t) {
        ...
        for (int q = 0; q < n_obs; ++q) {
          if (p_old_idx[q] - 1 == mod_idx) {
            double dist = p_new_dist[q + t_offset];
            
            if (dist < current_min_dist[q]) {
              current_min_dist[q] = dist;
              p_result[q] = t + 1;
            }
          }
        }
  ...
  void build_sqdist(const double* p_data,
                    int n_obs,
                    const std::vector<double>& old_tess,
                    int old_c,
                    const std::vector<double>& new_tess,
                    int new_c,
                    const std::vector<int>& old_dim,
                    int old_d,
                    const std::vector<int>& new_dim,
                    int new_d,
                    const Eigen::MatrixXd& old_sqdist,
                    const std::string& mod,
                    int mod_idx,
                    Eigen::MatrixXd& out_sqdist) {
    (void)old_d;
    
    out_sqdist.resize(n_obs, new_c);
    
    double* p_new = out_sqdist.data();
    const double* p_old = old_sqdist.data();
    
    if (mod == "RC") {
      if (mod_idx > 0) {
        std::memcpy(p_new, p_old, mod_idx * n_obs * sizeof(double));
      }
      
      if (mod_idx < old_c - 1) {
        std::memcpy(
          p_new + (mod_idx * n_obs),
          p_old + ((mod_idx + 1) * n_obs),
          ((old_c - 1) - mod_idx) * n_obs * sizeof(double)
        );
      }
  ...
  void build_weights(const Eigen::MatrixXd& sqdist,
                     int n_dims,
                     double p_val,
                     Eigen::MatrixXd& out_weights,
    ...
    for (int i = 0; i < n_obs; ++i) {
      ...
      for (int c = 0; c < n_centres; ++c) {
        if (sqdist(i, c) * dim_scale < eps_sq) {
          is_zero = true;
          z_idx = c;
          break;
        }
      ...
      if (is_zero) {
        for (int c = 0; c < n_centres; ++c) {
          out_weights(i, c) = (c == z_idx) ? 1.0 : 0.0;
        }
      } else {
        double rsum = 0.0;
        ...
        for (int c = 0; c < n_centres; ++c) {
          ...
          double w =
            fast_path
            ? 1.0 / (sqd + epsilon)
              : 1.0 / (std::pow(sqd, half_power) + epsilon);
          
          out_weights(i, c) = w;
          rsum += w;
        }
        
        double inv_sum = 1.0 / fmax(rsum, 1e-16);
        
        for (int c = 0; c < n_centres; ++c) {
          out_weights(i, c) *= inv_sum;
        }
      }
    }
  }
  
  PosteriorStats build_stats(const Eigen::MatrixXd& w,
    ...
    if (llt.info() == Eigen::Success) {
      ...
      stats.log_det_Q =
        2.0 * stats.L_chol.diagonal().array().log().sum();
      stats.valid = true;
    }
  ...
  void update_mu_soft(Tessellation& t) {
    ...
    for (int i = 0; i < t.n_centres; ++i) {
      z(i) = norm_rand();
    }
    
    t.mu =
      t.current_stats.mu_post
    + t.current_stats.L_chol
       .triangularView<Eigen::Lower>()
       .transpose()
       .solve(z);
  }
  
  SEXP propose_tessellation_cpp(SEXP tess_j_sexp,
                                SEXP dim_j_sexp,
                                SEXP sd_sexp,
                                SEXP mu_sexp,
                                SEXP num_cov_sexp,
                                SEXP s_weights_sexp) {
    const double* p_tess_j = REAL(tess_j_sexp);
    const int* p_dim_j = INTEGER(dim_j_sexp);
    const double* sd = REAL(sd_sexp);
    const double* mu = REAL(mu_sexp);
    int numCovariates = INTEGER(num_cov_sexp)[0];
    const double* p_s_weights = REAL(s_weights_sexp);
    
    int mu_len = Rf_length(mu_sexp);
    int sd_len = Rf_length(sd_sexp);
    
    auto get_mu = [&](int idx) {
      return mu_len > 1 ? mu[idx] : mu[0];
    };
    
    auto get_sd = [&](int idx) {
      return sd_len > 1 ? sd[idx] : sd[0];
    };
    ...
    std::vector<double> tess_j_star(
        p_tess_j,
        p_tess_j + (tess_j_rows * d_j_length)
    );
    
    std::string modification = "Change";
    int modified_idx = 0;
    double p_move = unif_rand();
    
    if ((p_move < 0.2 && d_j_length != numCovariates) ||
        (d_j_length == 1 && d_j_length != numCovariates && p_move < 0.4)) {
      modification = "AD";
      ...
      for (int i = 1; i <= numCovariates; ++i) {
        if (!in_vector(i, dim_j_star)) {
          ...
          sum_w += fmax(p_s_weights[i - 1], 1e-16);
        }
      }
      
      if (eligible.empty()) {
        Rf_error("No eligible covariates available for AD proposal.");
      }
      ...
      if (eligible.size() > 1) {
        ...
        for (int e : eligible) {
          cum += fmax(p_s_weights[e - 1], 1e-16);
      ...
      for (int r = 0; r < tess_j_rows; ++r) {
        for (int c = 0; c < d_j_length; ++c) {
          new_tess[r + c * tess_j_rows] =
            tess_j_star[r + c * tess_j_rows];
        }
        
        new_tess[r + d_j_length * tess_j_rows] =
          get_mu(new_dim - 1) + norm_rand() * get_sd(new_dim - 1);
      }
      
      tess_j_star = new_tess;
      
    } else if (p_move < 0.4 && d_j_length > 1) {
      modification = "RD";
      
      int removed_dim_idx =
        static_cast<int>(unif_rand() * d_j_length);
      
      if (removed_dim_idx < 0) {
        removed_dim_idx = 0;
      }
      
      if (removed_dim_idx >= d_j_length) {
        removed_dim_idx = d_j_length - 1;
      }
      ...
      for (int old_col = 0; old_col < d_j_length; ++old_col) {
        if (old_col == removed_dim_idx) {
          continue;
        }
        
        for (int r = 0; r < tess_j_rows; ++r) {
          new_tess[r + new_col * tess_j_rows] =
            tess_j_star[r + old_col * tess_j_rows];
        }
    ...
    } else if (p_move < 0.6 || (p_move < 0.8 && tess_j_rows == 1)) {
      modification = "AC";
      modified_idx = tess_j_rows + 1;
      
      for (int i = 0; i < d_j_length; ++i) {
        int cov_idx = dim_j_star[i] - 1;
        
        tess_j_star.insert(
          tess_j_star.begin() + (i * (tess_j_rows + 1)) + tess_j_rows,
          get_mu(cov_idx) + norm_rand() * get_sd(cov_idx)
        );
      }
      
    } else if (p_move < 0.8 && tess_j_rows > 1) {
      modification = "RC";
      modified_idx = static_cast<int>(floor(unif_rand() * tess_j_rows));
      
      if (modified_idx < 0) modified_idx = 0;
      if (modified_idx >= tess_j_rows) modified_idx = tess_j_rows - 1;
    ...
    } else if (p_move < 0.9 || d_j_length == numCovariates) {
      modification = "Change";
      modified_idx = static_cast<int>(floor(unif_rand() * tess_j_rows));
      ...
      for (int c = 0; c < d_j_length; ++c) {
        int cov_idx = dim_j_star[c] - 1;
        
        tess_j_star[modified_idx + c * tess_j_rows] =
          get_mu(cov_idx) + norm_rand() * get_sd(cov_idx);
      }
      ...
      modification = "Swap";
      modified_idx = static_cast<int>(floor(unif_rand() * d_j_length));
      
      if (modified_idx < 0) modified_idx = 0;
      if (modified_idx >= d_j_length) modified_idx = d_j_length - 1;
      ...
      if (eligible.empty()) {
        Rf_error("No eligible covariates available for Swap proposal.");
      }
      ...
      for (int r = 0; r < tess_j_rows; ++r) {
        tess_j_star[r + modified_idx * tess_j_rows] =
          get_mu(new_dim - 1) + norm_rand() * get_sd(new_dim - 1);
      }
    ...
    SEXP res_tess_j_star;
    SEXP res_dim_j_star;
    SEXP res_mod;
    SEXP res_row_column_modified;
    SEXP result_list;
    SEXP list_names;
    
    int new_rows =
      static_cast<int>(tess_j_star.size() / dim_j_star.size());
    
    PROTECT(res_tess_j_star =
      Rf_allocMatrix(REALSXP, new_rows, dim_j_star.size()));
    
    std::memcpy(
      REAL(res_tess_j_star),
      tess_j_star.data(),
      tess_j_star.size() * sizeof(double)
    );
    
    PROTECT(res_dim_j_star =
      Rf_allocVector(INTSXP, dim_j_star.size()));
    
    std::memcpy(
      INTEGER(res_dim_j_star),
      dim_j_star.data(),
      dim_j_star.size() * sizeof(int)
    );
...
extern "C" {
  ...
  SEXP super_mcmc_loop_cpp(SEXP x_sexp,
                           SEXP y_sexp,
                           SEXP sum_sexp,
                           SEXP tess_list,
                           SEXP dim_list,
                           ...
                           SEXP sigma2mu_sexp,
                           SEXP omega_sexp,
                           SEXP poisson_lambda_sexp,
                           SEXP total_iter_sexp,
                           SEXP burn_in_sexp,
                           SEXP thinning_sexp,
                           SEXP nu_sexp,
                           SEXP invgamma_lambda_sexp,
                           SEXP show_progress_sexp,
                           SEXP alpha_sexp,
                           SEXP a_alpha_sexp,
                           SEXP b_alpha_sexp,
                           SEXP rho_alpha_sexp,
                           SEXP update_alpha_sexp,
                           SEXP dirichlet_warmup_sexp,
                           SEXP adapt_boost_sexp,
                           SEXP adapt_penalty_sexp,
                           SEXP momentum_decay_sexp,
                           SEXP kappa_sexp,
                           SEXP tau_sexp,
                           SEXP var_sel_mode_sexp,
                           SEXP split_mode_sexp,
                           SEXP power_sexp,
                           SEXP p_shape_sexp,
                           SEXP p_rate_sexp,
                           SEXP p_sd_sexp,
    ...
    double kappa = REAL(kappa_sexp)[0];
    double tau = REAL(tau_sexp)[0];
    ...
    if (split_mode == 1) {
      ...
      PROTECT(return_indices = Rf_allocVector(VECSXP, 1));
    }
    ...
    if (split_mode == 0) {
      out_power = PROTECT(Rf_allocMatrix(REALSXP, m, num_stored));
      out_mu = PROTECT(Rf_allocVector(VECSXP, num_stored));
    } else {
      out_power = PROTECT(Rf_allocVector(REALSXP, 1));
      out_mu = PROTECT(Rf_allocVector(VECSXP, 1));
    }
    ...
    std::vector<double> workspace_dist;
    std::vector<int> workspace_idx(n_obs);
    
    if (split_mode == 1) {
      workspace_dist.reserve(n_obs * 50);
    }
    
    Eigen::MatrixXd workspace_sqdist;
    Eigen::MatrixXd workspace_weights;
    std::vector<Tessellation> ensemble;
    
    Eigen::Map<Eigen::VectorXd> Y(Y_latent.data(), n_obs);
    Eigen::VectorXd Y_hat(n_obs);
    std::vector<Eigen::VectorXd> Forest_Preds;
    
    if (split_mode == 0) {
      workspace_sqdist.resize(n_obs, 50);
      workspace_weights.resize(n_obs, 50);
      ensemble.resize(m);
      ...
      for (int j = 0; j < m; ++j) {
        ...
        ensemble[j].tess.assign(
            REAL(rtess),
            REAL(rtess) + (ensemble[j].n_centres * ensemble[j].n_dims)
        );
        
        ensemble[j].dim.assign(
            INTEGER(rdim),
            INTEGER(rdim) + ensemble[j].n_dims
        );
    ...
    for (int iter = 0; iter < total_iter; ++iter) {
      ...
      if (show_progress &&
          (iter + 1) % std::max(1, total_iter / 100) == 0) {
        Rprintf(
          "\rProcessing MCMC Iteration %d of %d...",
          iter + 1,
          total_iter
        );
        R_FlushConsole();
      ...
      if (is_classification == 1) {
        for (int i = 0; i < n_obs; ++i) {
          double current_mean =
            (split_mode == 1) ? p_sum[i] : Y_hat(i);
          
          double u = unif_rand();
          
          if (original_y[i] == 1.0) {
            double prob_lower =
              R::pnorm5(0.0, current_mean, 1.0, 1, 0);
            
            double q =
              prob_lower + u * (1.0 - prob_lower);
            
            q = fmin(1.0 - 1e-15, fmax(1e-15, q));
            
            Y_latent[i] =
              R::qnorm5(q, current_mean, 1.0, 1, 0);
          } else {
            double prob_upper =
              R::pnorm5(0.0, current_mean, 1.0, 1, 0);
            
            double q = u * prob_upper;
          ...
          if (std::isnan(Y_latent[i]) || std::isinf(Y_latent[i])) {
            Y_latent[i] =
              (original_y[i] == 1.0)
            ? std::max(0.001, current_mean)
              : std::min(-0.001, current_mean);
          }
        }
        ...
        if (split_mode == 1) {
          hypers.sigma2 =
            sample_sigma_squared_cpp(
              n_obs,
              hypers.nu,
              hypers.lambda_invgamma,
              Y_latent.data(),
              p_sum
            );
        } else {
      ...
      for (int j = 0; j < m; ++j) {
        SEXP newTessOutput =
          propose_tessellation_cpp(
            VECTOR_ELT(return_tess, j),
            VECTOR_ELT(return_dim, j),
            sd_sexp,
            mu_sexp,
            p_sexp,
            s_weights_sexp
        ...
        int num_levels_old =
          Rf_nrows(VECTOR_ELT(return_tess, j));
        
        int num_centres_new =
          Rf_nrows(tess_j_star_sexp);
        
        int new_dims =
          Rf_length(dim_j_star_sexp);
        
        std::string mod =
          CHAR(STRING_ELT(mod_sexp, 0));
        
        int mod_idx_cpp =
          INTEGER(rcm_sexp)[0] - 1;
        
        int old_dims =
          Rf_length(VECTOR_ELT(return_dim, j));
        
        double log_varsel_struct_ratio = 0.0;
        
        if (var_sel_mode > 0) {
          log_varsel_struct_ratio =
            log_varsel_struct_ratio_modelB(
              INTEGER(VECTOR_ELT(return_dim, j)),
              old_dims,
              INTEGER(dim_j_star_sexp),
              new_dims,
              p_s_weights,
              mod,
              mod_idx_cpp,
        ...
        if (split_mode == 1) {
          ...
          const double* p_old_pred = REAL(old_pred_sexp);
          
          SEXP old_idx_sexp = VECTOR_ELT(return_indices, j);
          const int* p_old_idx = INTEGER(old_idx_sexp);
          
          double* p_R_j = R_j.data();
          
          for (int i = 0; i < n_obs; ++i) {
            p_sum[i] -= p_old_pred[p_old_idx[i] - 1];
            p_R_j[i] = Y_latent[i] - p_sum[i];
          }
          
          if (workspace_dist.capacity() <
            static_cast<size_t>(n_obs * num_centres_new)) {
            workspace_dist.reserve(n_obs * num_centres_new * 2);
          }
          
          workspace_dist.resize(n_obs * num_centres_new);
          
          calculate_distances_hard(
            p_x,
            REAL(VECTOR_ELT(return_tess, j)),
            REAL(tess_j_star_sexp),
            REAL(VECTOR_ELT(return_sqdist, j)),
            p_old_idx,
            INTEGER(VECTOR_ELT(return_dim, j)),
            INTEGER(dim_j_star_sexp),
            1,
          ...
          const int* p_new_idx = workspace_idx.data();
          ...
          for (int c = 0; c < num_levels_old; ++c) {
            denomOld[c] =
              fmax(hypers.sigma2mu * nIjOld[c] + hypers.sigma2, 1e-10);
          }
          
          for (int c = 0; c < num_centres_new; ++c) {
            denomNew[c] =
              fmax(hypers.sigma2mu * nIjNew[c] + hypers.sigma2, 1e-10);
          }
          
          log_prob =
            calc_acceptance_hard(
              rIjOld,
              denomOld,
              rIjNew,
              denomNew,
          ...
          ensemble[j].current_stats =
            build_stats(ensemble[j].cached_weights, res, hypers);
          
          std::vector<double> prop_tess(
              REAL(tess_j_star_sexp),
              REAL(tess_j_star_sexp) + (num_centres_new * new_dims)
          );
          
          std::vector<int> prop_dim(
              INTEGER(dim_j_star_sexp),
              INTEGER(dim_j_star_sexp) + new_dims
          );
          ...
          PosteriorStats prop_stats =
            build_stats(workspace_weights, res, hypers);
          
          if (prop_stats.valid && ensemble[j].current_stats.valid) {
            log_prob =
              calc_acceptance_soft(
                ensemble[j].current_stats,
                prop_stats,
                num_levels_old,
                num_centres_new,
          ...
          if (log_prob > -1e10) {
            ensemble[j].current_stats = std::move(prop_stats);
          }
        }
        ...
        if (var_sel_mode == 2) {
          ...
          const int* p_old_dim2 = INTEGER(VECTOR_ELT(return_dim, j));
          const int* p_new_dim2 = INTEGER(dim_j_star_sexp);
          
          if (mod == "AD") {
            adaptive_momentum[p_new_dim2[mod_idx_cpp] - 1] +=
              current_boost * acceptance_prob;
            
          } else if (mod == "RD") {
            adaptive_momentum[p_old_dim2[mod_idx_cpp] - 1] +=
              current_boost * (1.0 - acceptance_prob)
            - current_penalty * acceptance_prob;
        ...
        if (log(unif_rand()) < log_prob) {
          ...
          if (split_mode == 1) {
            SEXP new_indices_sexp =
              PROTECT(Rf_allocMatrix(INTSXP, n_obs, 1));
            
            std::memcpy(
              INTEGER(new_indices_sexp),
              workspace_idx.data(),
              n_obs * sizeof(int)
            );
            
            SET_VECTOR_ELT(return_indices, j, new_indices_sexp);
            
            SEXP new_sqdist_sexp =
              PROTECT(Rf_allocMatrix(REALSXP, n_obs, num_centres_new));
            
            std::memcpy(
              REAL(new_sqdist_sexp),
              workspace_dist.data(),
              n_obs * num_centres_new * sizeof(double)
            );
            
            SET_VECTOR_ELT(return_sqdist, j, new_sqdist_sexp);
            ...
            SEXP new_pred_sexp =
              PROTECT(Rf_allocMatrix(REALSXP, num_centres_new, 1));
            
            std::memcpy(
              REAL(new_pred_sexp),
              new_mu.data(),
              num_centres_new * sizeof(double)
            );
            ...
            ensemble[j].n_centres = num_centres_new;
            ensemble[j].n_dims = new_dims;
            
            ensemble[j].cached_sqdist = std::move(workspace_sqdist);
            ensemble[j].cached_weights = std::move(workspace_weights);
          ...
          if (split_mode == 1) {
            ...
            SEXP new_pred_sexp =
              PROTECT(Rf_allocMatrix(REALSXP, num_levels_old, 1));
            
            std::memcpy(
              REAL(new_pred_sexp),
              old_mu.data(),
              num_levels_old * sizeof(double)
            );
        ...
        if (split_mode == 1) {
          ...
          const double* p_active_pred = REAL(active_pred_sexp);
          
          SEXP active_idx_sexp = VECTOR_ELT(return_indices, j);
          const int* p_active_idx = INTEGER(active_idx_sexp);
          ...
          double log_p_star =
            log(ensemble[j].p_val) + norm_rand() * hypers.p_sd;
          ...
          if (prop_stats.valid && ensemble[j].current_stats.valid) {
            double ll_old =
              0.5 * ensemble[j].current_stats.quad_form
            - 0.5 * ensemble[j].current_stats.log_det_Q;
            
            double ll_new =
            0.5 * prop_stats.quad_form
            - 0.5 * prop_stats.log_det_Q;
            
            double prior_old =
            R::dexp(ensemble[j].p_val, 1.0 / hypers.p_rate, 1);
            
            double prior_new =
              R::dexp(p_star, 1.0 / hypers.p_rate, 1);
            
            if (log(unif_rand()) <
              (ll_new - ll_old
                 + prior_new - prior_old
                 + log_p_star - log(ensemble[j].p_val))) {
                 ensemble[j].p_val = p_star;
              ensemble[j].cached_weights = std::move(workspace_weights);
              ensemble[j].current_stats = std::move(prop_stats);
            }
          }
          ...
          Forest_Preds[j] =
            ensemble[j].cached_weights * ensemble[j].mu;
      ...
      if (var_sel_mode > 0 && iter > dirichlet_warmup) {
        ...
        for (int j = 0; j < m; ++j) {
          ...
          for (int k = 1; k < d_len; ++k) {
            ...
            for (int e = 0; e < k; ++e) {
              int var_id = latent_order[e];
              ineligible.push_back(var_id);
              sum_inel += p_s_weights[var_id - 1];
            }
            
            double prob_success =
              fmax(1e-5, fmin(1.0 - 1e-10, 1.0 - sum_inel));
            
            double raw_failures = R::rgeom(prob_success);
            int failures =
              static_cast<int>(fmin(raw_failures, 1000.0));
            
            if (failures > 0) {
              if (ineligible.size() == 1) {
                current_m_counts[ineligible[0] - 1] += failures;
                
              } else {
                int k_classes =
                  static_cast<int>(ineligible.size());
                
                std::vector<double> probs(k_classes);
                
                double sum_w = 0.0;
                
                for (int i = 0; i < k_classes; ++i) {
                  probs[i] = p_s_weights[ineligible[i] - 1];
                  sum_w += probs[i];
                }
                
                sum_w = fmax(sum_w, 1e-16);
                
                double running_sum = 0.0;
                
                for (int i = 0; i < k_classes - 1; ++i) {
                  probs[i] /= sum_w;
                  running_sum += probs[i];
                }
                
                probs[k_classes - 1] =
                  fmax(0.0, 1.0 - running_sum);
                
                std::vector<int> alloc_counts(k_classes, 0);
                ...
                for (int i = 0; i < k_classes; ++i) {
                  current_m_counts[ineligible[i] - 1] += alloc_counts[i];
                }
              }
            }
          }
        ...
        if (update_alpha == 1) {
          ...
          for (int i = 0; i < p; ++i) {
            C_total += current_m_counts[i];
          }
          
          auto eval_log_post_alpha = [&](double a_val) {
            double safe_a = fmax(a_val, 1e-12);
            
            double log_lik =
              R::lgammafn(safe_a)
              - R::lgammafn(safe_a + C_total);
            
            for (int i = 0; i < p; ++i) {
              if (current_m_counts[i] > 0.0) {
                log_lik +=
                  R::lgammafn((safe_a / p) + current_m_counts[i])
                - R::lgammafn(safe_a / p);
              }
            }
            
            double log_prior =
              (a_alpha - 1.0) * log(safe_a)
              - b_alpha * safe_a;
            
            return log_lik + log_prior;
          };
          
          double log_prob_star = eval_log_post_alpha(alpha_star);
          double log_prob_orig = eval_log_post_alpha(alpha);
          
          if (log(unif_rand()) <
            log_prob_star - log_prob_orig
                + log(alpha_star) - log(alpha)) {
            alpha = alpha_star;
          }
        }
        ...
        for (int i = 0; i < p; ++i) {
          ...
          if (var_sel_mode == 2) {
            ...
            raw_shape += safe_momentum;
          }
          
          raw_shape = fmax(raw_shape, 1e-10);
          
          p_s_weights[i] =
            fmax(R::rgamma(raw_shape, 1.0), 1e-16);
          
          sum_s_prop += p_s_weights[i];
        }
        
        sum_s_prop = fmax(sum_s_prop, 1e-16);
        
        for (int i = 0; i < p; ++i) {
          p_s_weights[i] /= sum_s_prop;
      ...
      if (iter >= burn_in &&
        ...
        if (split_mode == 0) {
          ...
          for (int j = 0; j < m; ++j) {
            ...
            std::memcpy(
              REAL(out_t),
              ensemble[j].tess.data(),
              ensemble[j].tess.size() * sizeof(double)
            );
            
            SEXP out_d =
              PROTECT(Rf_allocVector(INTSXP, ensemble[j].n_dims));
            
            std::memcpy(
              INTEGER(out_d),
              ensemble[j].dim.data(),
              ensemble[j].dim.size() * sizeof(int)
            );
            
            SEXP out_p =
              PROTECT(Rf_allocMatrix(REALSXP, n_obs, 1));
            ...
            std::memcpy(
              REAL(out_m),
              ensemble[j].mu.data(),
              ensemble[j].n_centres * sizeof(double)
            );
            
            SET_VECTOR_ELT(tl, j, out_t);
            SET_VECTOR_ELT(dl, j, out_d);
            SET_VECTOR_ELT(pl, j, out_p);
            SET_VECTOR_ELT(ml, j, out_m);
            
            UNPROTECT(4);
            
            REAL(out_power)[j + store_idx * m] =
              ensemble[j].p_val;
          }
          ...
          SET_VECTOR_ELT(out_mu, store_idx, ml);
        ...
        for (int i = 0; i < p; ++i) {
          REAL(out_dirichlet)[i + store_idx * p] =
            p_s_weights[i];
        }
        ...
        for (int i = 0; i < p; ++i) {
          INTEGER(out_var_sel)[i + store_idx * p] =
            current_covariate_counts[i];
        }
    ...
    PROTECT(return_list = Rf_allocVector(VECSXP, 12));
    ...
    SET_VECTOR_ELT(return_list, 11, out_momentum);
    
    PROTECT(list_names = Rf_allocVector(STRSXP, 12));
    ...
    SET_STRING_ELT(list_names, 11, Rf_mkChar("posteriorMomentum"));
  ...
  /*
   ...
   ---------------------------------------------------------
   */
  SEXP soft_predict_cpp(SEXP tess_sexp,
                        SEXP query_sexp,
                        SEXP dim_sexp,
                        SEXP mu_sexp,
                        SEXP power_sexp) {
    const double* p_tess = REAL(tess_sexp);
    const double* p_query = REAL(query_sexp);
    const int* dim_p = INTEGER(dim_sexp);
    const double* p_mu = REAL(mu_sexp);
    ...
    for (int d = 0; d < n_dims; ++d) {
      int cov_idx = dim_p[d] - 1;
      int query_offset = cov_idx * query_rows;
      int tess_offset = d * tess_rows;
      
      for (int c = 0; c < tess_rows; ++c) {
        double tess_val = p_tess[c + tess_offset];
        int dist_offset = c * query_rows;
        
        for (int q = 0; q < query_rows; ++q) {
          double diff = p_query[q + query_offset] - tess_val;
          dists[q + dist_offset] += diff * diff;
        }
      }
    }
    
    SEXP result =
      PROTECT(Rf_allocMatrix(REALSXP, query_rows, 1));
    ...
    for (int q = 0; q < query_rows; ++q) {
      ...
      for (int c = 0; c < tess_rows; ++c) {
        if (dists[q + c * query_rows] * dim_scale < eps_sq) {
          is_zero = true;
          z_idx = c;
          break;
        }
      ...
      if (is_zero) {
        ...
        for (int c = 0; c < tess_rows; ++c) {
          double sqd =
            dists[q + c * query_rows] * dim_scale;
        ...
        p_result[q] = pred_val / fmax(rsum, 1e-16);
      }
    }
  ...
  /*
   ...
   ---------------------------------------------------------
   */
  SEXP knnx_index_predict_cpp(SEXP tess_sexp,
                              SEXP query_sexp,
                              SEXP k_sexp,
                              SEXP dim_sexp) {
    const double* p_tess = REAL(tess_sexp);
    ...
    SEXP result =
      PROTECT(Rf_allocMatrix(INTSXP, query_rows, k));
    
    int* p_result = INTEGER(result);
    
    if (k == 1) {
      std::vector<double> current_min_dist(
          dists.begin(),
          dists.begin() + query_rows
      );
      ...
      for (int q = 0; q < query_rows; ++q) {
        for (int c = 0; c < tess_rows; ++c) {
          distances[c].first = dists[q + c * query_rows];
          distances[c].second = c + 1;
        }
