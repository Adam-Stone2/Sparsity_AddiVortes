...
# EFFICIENCY: Removed SoftBart (unused) and flexBART from worker setup
clusterEvalQ(cl, {
  devtools::load_all()
  library(BART)
  library(ranger)
...
### CV Hyperparameters ###
{
  ...
  # --- flexBART GRID (COMMENTED OUT) ---
  ## base_grid_flex    <- expand.grid(M_vec = c(20, 200))
  ## nu_q_cases_flex   <- data.frame(nu = c(3, 3, 10), q = c(0.90, 0.99, 0.75))
  ## alpha_beta_cases  <- data.frame(alpha = c(0.95, 0.8), beta = c(2.0, 3.0))
  ## core_grid         <- merge(base_grid_flex, nu_q_cases_flex, by = NULL)
  ## core_grid         <- merge(core_grid, alpha_beta_cases, by = NULL)
  ## grid_dense        <- core_grid; grid_dense$sparse <- FALSE
  ## grid_dense$a_u    <- 1.0; grid_dense$b_u <- 1.0
  ## sparsity_cases    <- data.frame(a_u = c(0.1, 0.5, 1.0), b_u = c(1.0))
  ## grid_sparse       <- merge(core_grid, sparsity_cases, by = NULL)
  ## grid_sparse$sparse <- TRUE
  ## grid_flexBART     <- rbind(grid_dense, grid_sparse)
  
  # --- wBART GRID (sparse = TRUE applied at call sites) ---
...
# Drop constant columns dynamically per CV fold
...
for (data_name in names(benchmark_datasets)) {
  ...
  if (is_classification) {
    ...
    # FIX: use integer 0/1 — avoids factor→1/2 coercion issues in XGBoost
  ...
  # EFFICIENCY: removed flexBART / SoftBart from packages_list
  ...
  cv_results_0 <- foreach(i = 1:nrow(grid_mode0), .combine = rbind,
                          .packages = packages_list) %dopar% {
                            ...
                            for (f in 1:5) {
                              ...
                              model <- AddiVortes(cv$Y_train, cv$X_train,
                                                  ...
                                                  Omega = params$Omega, LambdaRate = params$LambdaRate,
                                                  nu = params$nu, q = params$q,
                              ...
                              fold_score[f] <- if (is_classification) mean(round(preds) == cv$Y_val) else
  ...
  cv_results_1 <- foreach(i = 1:nrow(grid_mode1), .combine = rbind,
                          .packages = packages_list) %dopar% {
                            params     <- grid_mode1[i, ]
                            fold_score <- numeric(5)
                            for (f in 1:5) {
  ...
  cv_results_2 <- foreach(i = 1:nrow(grid_mode2), .combine = rbind,
                          .packages = packages_list) %dopar% {
                            params     <- grid_mode2[i, ]
                            fold_score <- numeric(5)
                            for (f in 1:5) {
  ...
  # --------------------------------------------------------------------
  # TUNING: flexBART (COMMENTED OUT)
  # --------------------------------------------------------------------
  ## cat("Tuning flexBART...\n")
  ## cv_results_flex <- foreach(i = 1:nrow(grid_flexBART), .combine = rbind,
  ##                            .packages = packages_list) %dopar% {
  ...
  ##     model <- flexBART(Y ~ bart(.), a = params$a_u, b = params$b_u,
  ##                       train_data = train_cv_set, sparsity = params$sparse,
  ##                       M_vec = params$M_vec, save_trees = TRUE,
  ##                       alpha_vec = params$alpha, beta_vec = params$beta,
  ##                       nu = params$nu, sigquant = params$q,
  ##                       n.chains = 1, sparse = TRUE,
  ##                       burn = MCMC_BURNIN_CV, nd = MCMC_ITER_CV - MCMC_BURNIN_CV)
  ##     preds <- colMeans(predict.flexBART(model, val_cv_set))
  ##     fold_score[f] <- if (is_classification) mean(ifelse(preds > 0.5, 1, 0) == cv$Y_val) else
  ##                       sqrt(mean((preds - cv$Y_val)^2))
  ##   }
  ##   data.frame(params, avg_score = mean(fold_score))
  ## }
  ## best_params_flex <- if (is_classification) cv_results_flex[which.max(cv_results_flex$avg_score), ] else
  ##                                            cv_results_flex[which.min(cv_results_flex$avg_score), ]
  
  # --------------------------------------------------------------------
  # TUNING: wBART  (sparse = TRUE)
  # --------------------------------------------------------------------
  cat("Tuning wBART...\n")
  cv_results_wbart <- foreach(i = 1:nrow(grid_wBART), .combine = rbind,
                              .packages = packages_list) %dopar% {
                                params     <- grid_wBART[i, ]
                                fold_score <- numeric(5)
                                for (f in 1:5) {
                                  ...
                                  if (is_classification) {
                                    model <- pbart(cv$X_train, cv$Y_train, cv$X_val,
                                                   ...
                                                   a=params$a,
                                                   k=params$k,
                                                   ndpost = MCMC_ITER_CV - MCMC_BURNIN_CV,
                                                   nskip  = MCMC_BURNIN_CV,
                                                   nkeeptreedraws = 0,                # EFFICIENCY: skip storing train fits
                                                   printevery    = .Machine$integer.max) # EFFICIENCY: suppress I/O in parallel
  ...
  print(best_params_wbart)
  
  # --------------------------------------------------------------------
  # TUNING: softBART (COMMENTED OUT)
  # --------------------------------------------------------------------
  ## cat("Tuning softBART...\n")
  ## cv_results_soft <- foreach(i = 1:nrow(grid_softBART), .combine = rbind,
  ##                            .packages = packages_list) %dopar% {
  ...
  ##       fold_score[f] <- mean(ifelse(model$p_mean > 0.5, 1, 0) == cv$Y_val)
  ##     } else {
  ##       model <- softbart(cv$X_train, cv$Y_train, cv$X_val, hypers = hypers, opts = opts)
  ##       fold_score[f] <- sqrt(mean((model$y_hat_test_mean - cv$Y_val)^2))
  ##     }
  ##   }
  ##   data.frame(params, avg_score = mean(fold_score))
  ## }
  ## best_params_soft <- if (is_classification) cv_results_soft[which.max(cv_results_soft$avg_score), ] else
  ##                                            cv_results_soft[which.min(cv_results_soft$avg_score), ]
  
  # --------------------------------------------------------------------
  # TUNING: Random Forest (ranger)
  # --------------------------------------------------------------------
  cat("Tuning Random Forest...\n")
  cv_results_rf <- foreach(i = 1:nrow(grid_rf), .combine = rbind,
                           .packages = packages_list) %dopar% {
                             params <- grid_rf[i, ]
                             fold_score <- numeric(5)
                             
                             current_splitrule <- if (is_classification) {
                               ifelse(params$use_extratrees, "extratrees", "gini")
                             } else {
                             ...
                             for (f in 1:5) {
                               ...
                               if (is_classification) {
                                 model <- ranger(x = cv$X_train, y = as.factor(cv$Y_train),
                                                 ...
                                                 importance      = "none",      # EFFICIENCY: skip var importance
                                                 num.threads     = 1)           # EFFICIENCY: prevent thread thrashing
  ...
  # --------------------------------------------------------------------
  cat("Tuning SVM...\n")
  cv_results_svm <- foreach(i = 1:nrow(grid_svm), .combine = rbind,
                            .packages = packages_list) %dopar% {
                              params     <- grid_svm[i, ]
                              fold_score <- numeric(5)
                              for (f in 1:5) {
  ...
  # --------------------------------------------------------------------
  cat("Tuning XGBoost...\n")
  cv_results_xgb <- foreach(i = 1:nrow(grid_xgb), .combine = rbind,
                            .packages = packages_list) %dopar% {
                              params   <- grid_xgb[i, ]
                              fold_score <- numeric(5)
                              ...
                              for (f in 1:5) {
                                cv <- get_cv_data(X_train, Y_train, cv_folds[[f]])
                                # FIX: integer labels (0/1), not factor (which coerces to 1/2 in xgboost)
                                target_label <- if (is_classification) as.integer(cv$Y_train) else cv$Y_train
  ...
  # ====================================================================
  # FINAL EVALUATION  (n_runs independent train/test splits)
  # ====================================================================
  cat("Fitting final models on full training set...\n")
  
  evaluation_results <- foreach(run = 1:n_runs, .combine = rbind,
                                .packages = packages_list) %dopar% {
                                  ...
                                  final_Addi_0 <- AddiVortes(Y_train, X_train,
                                                             ...
                                                             nu         = best_params_0$nu,
                                                             q          = best_params_0$q,
                                                             totalMCMCIter = MCMC_ITER_FINAL,
                                                             mcmcBurnIn    = MCMC_BURNIN_FINAL,
                                                             numChains = 1, IntialSigma = "LASSO", varSelMode = 0)
                                  preds_0      <- predict(final_Addi_0, X_test, showProgress = FALSE)
                                  score_addi_0 <- if (is_classification) mean(round(preds_0) == Y_test) else
                                    sqrt(mean((preds_0 - Y_test)^2))
                                  ...
                                  final_Addi_1 <- AddiVortes(Y_train, X_train,
                                                             ...
                                                             nu         = best_params_1$nu,
                                                             q          = best_params_1$q,
                                                             a_alpha    = best_params_1$a_alpha,
                                                             b_alpha    = best_params_1$b_alpha,
                                                             totalMCMCIter   = MCMC_ITER_FINAL,
                                                             mcmcBurnIn      = MCMC_BURNIN_FINAL,
                                  ...
                                  score_addi_1 <- if (is_classification) mean(round(preds_1) == Y_test) else
                                    sqrt(mean((preds_1 - Y_test)^2))
                                  ...
                                  final_Addi_2 <- AddiVortes(Y_train, X_train,
                                                             ...
                                                             a_alpha       = best_params_2$a_alpha,
                                                             b_alpha       = best_params_2$b_alpha,
                                                             tau           = best_params_2$tau,
                                                             adaptBoost    = best_params_2$adaptBoost,
                                                             adaptPenalty  = best_params_2$adaptPenalty,
                                  ...
                                  score_addi_2 <- if (is_classification) mean(round(preds_2) == Y_test) else
                                    sqrt(mean((preds_2 - Y_test)^2))
                                  
                                  # ------------------------------------------------------------------
                                  # flexBART  (COMMENTED OUT)
                                  # ------------------------------------------------------------------
                                  ## final_flex <- flexBART(Y ~ bart(.), a = best_params_flex$a_u, b = best_params_flex$b_u,
                                  ##                        train_data = training_data, M_vec = best_params_flex$M_vec,
                                  ##                        save_trees = TRUE, n.chains = 1, sparse = best_params_flex$sparse,
                                  ##                        alpha_vec = best_params_flex$alpha, beta_vec = best_params_flex$beta,
                                  ##                        nu = best_params_flex$nu, sigquant = best_params_flex$q,
                                  ##                        burn = MCMC_BURNIN_FINAL, nd = MCMC_ITER_FINAL - MCMC_BURNIN_FINAL)
                                  ## preds_flex <- colMeans(predict.flexBART(final_flex,
                                  ##                        test_set[, c("Y", colnames(X_train)), drop = FALSE]))
                                  ## score_flex <- if (is_classification) mean(ifelse(preds_flex > 0.5, 1, 0) == Y_test) else
                                  ##               sqrt(mean((preds_flex - Y_test)^2))
                                  ...
                                  if (is_classification) {
                                    final_wbart <- pbart(X_train, Y_train, X_test,
                                                         ...
                                                         a=best_params_wbart$a,
                                                         k=best_params_wbart$k,
                                                         sparse = TRUE,
                                  ...
                                  ##   score_soft <- mean(ifelse(final_soft$p_mean > 0.5, 1, 0) == Y_test)
                                  ## } else {
                                  ##   final_soft <- softbart(X_train, Y_train, X_test, hypers = hypers, opts = opts)
                                  ##   score_soft <- sqrt(mean((final_soft$y_hat_test_mean - Y_test)^2))
                                  ## }
                                  ...
                                  obj_type     <- ifelse(is_classification, "binary:logistic", "reg:squarederror")
                                  # FIX: integer labels (0/1), not factor (which coerces to 1/2 in xgboost)
                                  target_label <- if (is_classification) as.integer(Y_train) else Y_train
                                  ...
                                  score_xgb <- if (is_classification) mean(ifelse(preds_xgb > 0.5, 1, 0) == Y_test) else
                                    sqrt(mean((preds_xgb - Y_test)^2))
                                  
                                  # ------------------------------------------------------------------
                                  # Baseline
                                  # ------------------------------------------------------------------
                                  baseline_score <- if (is_classification) {
                                    max(sum(Y_test == 0), sum(Y_test == 1)) / length(Y_test)
                                  } else {
                                    sqrt(mean((mean(Y_test) - Y_test)^2))
                                  }
...
average_spot<-rep(0,8)
for(i in 1:length(results_summary_full[,1])){
  print(order(as.vector(as.numeric(results_summary_full[i,2:9]))))
  #average_spot<-average_spot+order(as.vector(as.numeric(results_summary_full[i,2:9])))
  for(j in 1:8){
    print(which(order(as.vector(as.numeric(results_summary_full[i,2:9])))==j))
  average_spot[j]<-average_spot[j]+which(order(as.vector(as.numeric(results_summary_full[i,2:9])))==j)
  }
  
}

print(c('Addivortes','dirichlet','Adaptive','BART','RF','SVM','XGboost','Baseline'))
print(average_spot/length(results_summary_full[,1]))
